...
def plot_stability(input_path, output_path):
    ...
    ax_right = ax.twiny()
    sns.scatterplot(x, y, s=scatter_size, ax=ax, color="k", marker="+", label="Stability", legend=False, alpha=0.5)
    ax.grid(axis="x")
    ax.errorbar(x, y, yerr = yerr, xerr = xerr,  ls='', color="k", lw=lw, label="CI", alpha=0.5) 
    ...
    x = data["r2_mean"]
    xerr = data["r2_error"]
    sns.scatterplot(x, y, s=scatter_size, ax=ax_right, color="b", marker="x", label=r"$R^2$", legend=False, alpha=0.5)
    ax_right.errorbar(x, y, yerr = yerr, xerr = xerr,  ls='', color="b", lw=lw, label="CI", alpha=0.5) 
    ...
    fig.legend(ncol=2, loc="upper left", bbox_to_anchor=(-0.2,1.035),  bbox_transform=ax.transAxes, fontsize = "large")
