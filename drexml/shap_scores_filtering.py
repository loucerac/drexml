...
def process_data(data_folder, stability_threshold = 0.6):
    ...
    filtered_df_stable = filtered_df[filtered_df['circuit_name'].isin(stability_00['circuit_name'])]
    filtered_df_stable.to_csv(f"{data_folder}/shap_filtered_stability_symbol.tsv", sep='\t', index=False)