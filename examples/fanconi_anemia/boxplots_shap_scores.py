...
def create_boxplots(data_folder, filtered_file_name):
    ...
    medians = (
        melted_data.groupby("variable")["value"].median().sort_values(ascending=False)
    )

    # Order the data based on medians
    melted_data["variable"] = pd.Categorical(
        melted_data["variable"], categories=medians.index, ordered=True
    )
